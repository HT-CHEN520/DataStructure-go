...
type skipListNode struct {
	...
	level int
	//每层前进指针
	forwards []*skipListNode // forward 向前→
}
...
func (sl *SkipList) Insert(v interface{}, score int) int {
	...
	for ; i >= 0; i-- {
		for nil != cur.forwards[i] {
			if cur.forwards[i].v == v { // 节点值已存在
				return 2
			}
			if cur.forwards[i].score > score { // 当前层的下一个节点值大于待查值
				update[i] = cur // 将当前节点算入路径
				break           // 终止当前循环，继续查找下一层路径
			}
			cur = cur.forwards[i] // 其他情况，则在当前层链上前进一个节点
		}
		if nil == cur.forwards[i] { // 当前层还未有下一个节点
			update[i] = cur // 则将当前节点算作每一层的路径
		}
	}
	...
	//原有节点连接
	for i := 0; i <= level-1; i++ {
		next := update[i].forwards[i]   // next为当前层（i）当前路径的下一个节点
		update[i].forwards[i] = newNode // 将当前层当前路径的下一个节点更改为newNode
		newNode.forwards[i] = next      // 连接新节点和next
	}
